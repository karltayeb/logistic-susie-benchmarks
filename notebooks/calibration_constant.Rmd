...
pips <- pips %>% 
  ...
  select(y_name, fit_method, X_name, L, pip, causal)
...
### Calibration of SuSiEs PIPs under $L=1$ simulations
...
cs %>%
  ...
  make_coverage_plot(max_size=100)
```
### Coverage of SERs 95% Credible Sets

We can look at the coverage of all credible sets. But this also includes the credible sets of "null" SERs.
If $L$ is set larger than the number of actual signals we expect to estimate a few SERs that do not capture any signal, and thus return a very wide credible set.
Interestingly, for Logistic SuSiE, many of the CSs for these null SERs do NOT include the feature discovered in the non-null SERs. So we see that across all CSs,
logistic SuSiE seems to have poor coverage!

While we should have a principled way of selecting which SERs to include in the final model (a classick "picking K" problem),
for now we can get a pretty clear picture of what is going on by filtering down to smaller credible sets. When we filter down to credible sets of 10, or size 1,
we see that the CSs from linear SuSiE have poor coverage compared to logistic SuSiE. IBSS2M, which can take advantage of a richer variational approximation seems to perform best
(we'll see below, that it is not achieving good coverage by simply returning the trivial credible set).
...
cs %>%
  filter(stringr::str_detect(fit_method, c('susie|ibss'))) %>%
  make_coverage_plot(max_size=10)
```

```{r}
cs %>%
  filter(stringr::str_detect(fit_method, c('susie|ibss'))) %>%
  make_coverage_plot(max_size=2)
```
...
plots <- pips  %>%
  filter(stringr::str_detect(fit_method, c('ser'))) %>% 
  group_by(X_name) %>%
  group_map(~make_power_fdr_plot(.x, color=plot_colors))

purrr::lift_dl(cowplot::plot_grid)(plots, ncol=3)
```
...
plots <- pips %>%
  group_by(X_name) %>%
  group_map(~make_power_fdr_plot(.x, colors=plot_colors))
